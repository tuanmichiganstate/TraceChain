...
The first run adopts the existing activity as `SimuLedger Guided` and creates
`SimuLedger Practice`, `SimuLedger Challenge`, `SimuLedger Assessment`,
`SimuLedger Audit Guided`, `SimuLedger Audit Practice`,
`SimuLedger Audit Challenge`, `SimuLedger Audit Assessment`, and
`SimuLedger Technical Laboratory` beside it. Later runs update those nine
...
Exercises all activities with codec-shaped payloads (SL1 for Operations, TA1
...
nonzero while still passing cleanup:
    ...
    SIMULEDGER_ACCEPTANCE_FORCE_FAILURE=1 ./docker-moodle/run-acceptance.sh
