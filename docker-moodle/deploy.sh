...
"${compose[@]}" exec -T --user root moodle sh -c \
  'rm -f /tmp/TraceChain_Guided_*_NON_RELEASE.zip /tmp/TraceChain_Challenge_*_NON_RELEASE.zip /tmp/TraceChain_Assessment_*_NON_RELEASE.zip /tmp/deploy.php'
...
echo "--- deploy and reset Guided + Challenge + Assessment ---"
